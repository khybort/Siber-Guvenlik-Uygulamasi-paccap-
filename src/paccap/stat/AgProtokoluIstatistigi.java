...
public class AgProtokoluIstatistigi extends IstatistikAlici
{
	...
	public AgProtokoluIstatistigi(){
		analizciler=JPaketAnalizciYukleyici.getAnalyzersOf(JPaketAnalizci.NETWORK_LAYER);
	...
	public void analyze(List<Packet> paketler){
		for(int i=0;i<paketler.size();i++){
			...
			if(!flag){
				...
				paketBoyutu[paketBoyutu.length-1]+=p.len;
			}
		}
	}
